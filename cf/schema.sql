...
-- Рассказ автора о себе и ссылки на его места в сети — то, что стоит на его
-- странице под именем.
--
-- Взяться этому в библиотеке неоткуда: про автора сайт знает ровно то, что
-- написано в поле <author> внутри .siq, — одну подпись и ничего больше. Всё
-- остальное на его странице сайт СЧИТАЕТ: сколько паков, сколько игр, какие
-- оценки. Сказать же о себе словами — «пишу музыкальные паки, вопросы шлите
-- в Discord» — до сих пор было негде.
--
-- Ключ — canon_key, человек, а не написание подписи: «Vieldy» и «vieldy_» —
-- один автор, и рассказ у него один на все свои файлы (см. mergeAuthors
-- в src/indexer/authors.js). По той же причине ключ здесь тот же, что
-- у подписки на автора (author_subs выше).
--
-- Ссылок ровно две, и обе отдельными колонками, а не списком: значков на
-- странице автора два — Discord и Steam, — и каждый из них или есть, или нет.
-- Список «ссылка + подпись» превратил бы страницу автора в место, куда пишут
-- что угодно и сколько угодно; две именованные колонки этого не позволяют
-- по устройству. Что можно положить в каждую, решает не база, а проверка
-- при записи (см. cf/src/library/about.js): чужой адрес сюда не ложится вовсе.
--
-- Кто написал — рядом, и ровно затем же, зачем в pack_texts: правит рассказ
-- сам автор, а снять его должен уметь и хозяин сайта.
CREATE TABLE IF NOT EXISTS author_texts (
