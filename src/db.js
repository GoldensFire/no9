...
for (const [name, definition] of [
	...
	// На скольких паков разом посчитана статистика этого пака: сколько в базе
	// живых паков с тем же match_key, то есть названных автором одинаково.
	//
	// Число это и раньше считалось — подзапросом, на каждый пак показанной
	// страницы. Колонкой оно стало, когда понадобилось порядку выдачи: по числу
	// игр библиотека сортируется целиком, и подзапрос в ORDER BY считался бы
	// не на две дюжины паков, а на все пятнадцать тысяч — наверху это прочитанные
	// строки D1, считанные по тарифу. Подзапроса с тех пор нет вовсе: двух правд
	// об одном не держат (см. toPackage в src/server/packs.js).
	//
	...
	if (!existingColumns.has(name)) {
		...
		// Только что заведённое число тёзок сразу и считаем: до ближайшего
		// ночного обхода у всей базы стояла бы единица, и «популярные за всё
		// время» ровно столько же времени показывали бы то самое, ради чего
		// колонка и заводилась, — четырнадцать одинаково названных паков подряд
		if (name === 'stats_twins') {
			refreshStatsTwins();
		}
...
/**
 * Пересчитать, сколько у каждого пака тёзок, — то есть на сколько паков разом
 * сервис статистики посчитал его игры.
 *
 * ————— зачем это число хранить —————
 *
 * Сервис SIGame ищет пак по имени и авторам из файла, другого ключа у него нет,
 * и одинаково названные паки одного автора он складывает в одну кучу: у МакКайлы
 * четырнадцать «Вопросов SIGame», и на все четырнадцать вопросов сервис отвечает
 * одним и тем же числом. Карточка это уже учитывает и делит кучу поровну
 * (см. perPackGames в src/keys.js) — а порядок выдачи не учитывал, и «популярные
 * за всё время» открывались подряд идущими паками одного автора с одинаковым
 * числом игр на каждом. По ста девятнадцати играм на пак в топ не попадает
 * ни один из них; по полутора тысячам — все четырнадцать.
 *
 * Сортировке нужен не подзапрос, а колонка. Считать тёзок подзапросом можно
 * там, где паков две дюжины, — на показанной странице; в ORDER BY тот же
 * подзапрос считался бы на всю библиотеку целиком, а наверху прочитанные строки
 * идут по тарифу D1. Поэтому число теперь одно и лежит здесь, а показ читает
 * его оттуда же, откуда и порядок (см. toPackage в src/server/packs.js).
 *
 * ————— и почему пересчёт общий —————
 *
 * Тёзки — свойство не пака, а всей их компании: выложили пятнадцатый «Вопрос
 * SIGame» — и у четырнадцати прежних число поменялось, хотя их самих никто
 * не трогал. Поэтому и считается всё разом, без сужения до названных поимённо
 * паков: точечный пересчёт оставил бы соседей со вчерашним числом.
 *
 * Пишется при этом только изменившееся: строк тут пятнадцать тысяч, меняются
 * единицы, а каждая записанная уезжает наверх отдельной строкой заливки
 * (см. scripts/export-d1.js).
 *
 * Копии в счёт не идут: у них тот же match_key, но на сайте их нет вовсе,
 * и «статистика на двоих» у пака и его спрятанной копии было бы неправдой.
 *
 * @returns {number} сколько паков переписано
 */
export function refreshStatsTwins() {
	const counted = `(
		SELECT COUNT(*) FROM packages q
		WHERE q.status = 'ok' AND q.match_key IS NOT NULL AND q.match_key = packages.match_key
	)`;

	// У пака без ключа сравнения тёзок нет и быть не может: сервис статистики
	// его не искал вовсе. Единица, а не ноль, — на это число делят
	return db.prepare(`
		UPDATE packages SET stats_twins = MAX(1, COALESCE(${counted}, 1))
		WHERE stats_twins IS NOT MAX(1, COALESCE(${counted}, 1))
	`).run().changes;
}
...
// сервис SIGame сложил в одну кучу. Спрашивают об этом двое — пересчёт тёзок
// (см. refreshStatsTwins выше) и номер пака среди них на его собственной
// странице (см. TWIN_NO в src/server/packs.js), — и обоим нужны те же две
// колонки: сам ключ и «пак ещё на сайте».
//
